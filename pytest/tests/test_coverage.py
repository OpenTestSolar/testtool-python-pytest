...
def test_compute_source_list_without_env(monkeypatch):
    ...
    monkeypatch.setattr(
        os.path, "isdir", lambda x: x in ["package1", "package2", "test"]
    )
...
@patch(
    "coverage.data.CoverageData.measured_files",
    return_value=["/path/to/package1/file1.py"],
)
@patch(
    "coverage.data.CoverageData.contexts_by_lineno",
    return_value={10: ["test.package1::test_func|context"]},
)
@patch("builtins.open", new_callable=mock_open)
@patch("os.path.isfile", return_value=True)
def test_get_testcase_coverage_data(
    mock_isfile, mock_open, mock_contexts, mock_files, mock_read
):
...
def test_find_coverage_path_with_coveragerc(monkeypatch):
    ...
    monkeypatch.setattr(
        "builtins.open", mock_open(read_data="data_file = /path/to/coverage_db")
    )
    assert find_coverage_path("/path/to/proj", "coverage_db") == "coverage_db"
...
def test_find_coverage_path_with_walk(monkeypatch):
    ...
    assert (
        find_coverage_path("/path/to/proj", "coverage_db") == "/path/to/proj/.coverage"
    )
...
@patch("os.makedirs")
@patch(
    "testsolar_pytestx.extend.coverage_extend.find_coverage_path",
    return_value="/path/to/proj/.coverage",
)
@patch(
    ...
    return_value={
        "uttest.test_add.test_add_2_numbers": CoverageData(
            ...
            files={"addition_mod/add.py": [11, 12]},
        ),
...
def test_handle_coverage(
    mock_get_testcase_coverage_data,
    mock_find_coverage_path,
    mock_makedirs,
    mock_rmtree,
    mock_exists,
):
    ...
    assert (
        written_data["caseCoverage"][0]["caseName"]
        == "uttest.test_add.test_add_2_numbers"
    )
    assert (
        written_data["caseCoverage"][1]["caseName"] == "uttest.test_add.test_add_param"
    )
