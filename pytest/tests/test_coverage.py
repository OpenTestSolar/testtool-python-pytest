...
def test_coverage_to_json():
    ...
    # 创建 TestFileLines 对象
    test_file_lines1 = TestFileLines(
        fileName="file1.py",
        fileLines=[1, 2, 3]
    )
    test_file_lines2 = TestFileLines(
        fileName="file2.py",
        fileLines=[4, 5, 6]
    )
...
def test_convert_coverage_data():
    ...
    expected_result = {
        ...
        "test_case3": CoverageData(
            name="test_case3",
            files={
                "file2.py": [4, 5]
            }
        ),
        "test_case4": CoverageData(
            name="test_case4",
            files={
                "file2.py": [6]
            }
...
def test_get_testcase_coverage_data(monkeypatch):
    ...
    # 验证结果
    expected_result = {'uttest.test_add.test_add_param': CoverageData(name='uttest.test_add.test_add_param', files={'/data/tests/addition_mod/add.py': [11, 12]}), 'uttest.test_add.test_add_dict': CoverageData(name='uttest.test_add.test_add_dict', files={'/data/tests/addition_mod/add.py': [11, 13, 15, 17, 18, 19, 20, 21]}), 'uttest.test_add.test_add_2_numbers': CoverageData(name='uttest.test_add.test_add_2_numbers', files={'/data/tests/addition_mod/add.py': [11, 12]})}
...
class TestGenerateCoverageJsonFile:
    ...
    def setup_test_environment(self):
        ...
        cov_file_info = {
            "test_case1": CoverageData(
                name="test_case1",
                files={"file1.py": [1, 2, 3]}
            ),
            "test_case2": CoverageData(
                name="test_case2",
                files={"file2.py": [4, 5, 6]}
            ),
        }
    ...
    def test_generate_coverage_json_file(self, setup_test_environment, monkeypatch):
        proj_path, coverage_file_path, cov_file_info, coverage_json_file = setup_test_environment
...
class TestCollectCoverageReport:
    ...
    def test_collect_coverage_report(self, setup_test_environment, monkeypatch):
        proj_path, file_report_path, coverage_file_path, coverage_db_path = setup_test_environment

        # Mock dependencies
        filter_coverage_xml_packages = MagicMock()
        find_coverage_db_path = MagicMock(return_value=coverage_db_path)
        get_testcase_coverage_data = MagicMock(return_value={
            "test_case1": {
                "name": "test_case1",
                "files": {"file1.py": [1, 2, 3]}
            }
        ...
        monkeypatch.setattr("testsolar_pytestx.extend.coverage_extend.filter_coverage_xml_packages", filter_coverage_xml_packages)
        monkeypatch.setattr("testsolar_pytestx.extend.coverage_extend.find_coverage_db_path", find_coverage_db_path)
        monkeypatch.setattr("testsolar_pytestx.extend.coverage_extend.get_testcase_coverage_data", get_testcase_coverage_data)
        monkeypatch.setattr("testsolar_pytestx.extend.coverage_extend.generate_coverage_json_file", generate_coverage_json_file)
        ...
        # Assert calls
        filter_coverage_xml_packages.assert_called_once_with(coverage_file_path, ["package1", "package2"])
        find_coverage_db_path.assert_called_once_with(proj_path, ".coverage")
        get_testcase_coverage_data.assert_called_once_with(["package1", "package2"], coverage_db_path)
